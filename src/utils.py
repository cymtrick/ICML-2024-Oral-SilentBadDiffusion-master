...
def build_trigger_prompt(
    ...
    dataset_name: str,
    with_special_char: bool = False,
    spec_char: str = "*",
) -> str:
    """
    Build an inference prompt consistent with `SlientBadDiffusion_validation` in `target_model_training.py`.

    - Keeps only the prefix part of `caption` before `spec_char` (if present).
    - Appends phrases as a comma-joined list.
    - Adds dataset-specific suffix (Pokemon uses " in white background").
    """
    suffix = " in white background" if dataset_name.lower() == "pokemon" else ""
    txt_splitter = f" {spec_char}" if with_special_char else " "

    cooked_caption = caption
    if spec_char not in cooked_caption:
        # Match `target_model_training.SlientBadDiffusion_validation`: mark phrase occurrences with a special char
        # so we can cleanly split out the prefix part.
        for ph in phrases:
            if ph in cooked_caption:
                cooked_caption = cooked_caption.replace(ph, f"{spec_char}{ph}")

    prefix = cooked_caption.split(spec_char)[0].strip()
    prompt = prefix + ", ".join([txt_splitter + ph for ph in phrases]) + suffix
    return prompt.replace("  ", " ")


def load_synonym_mapping(path: Optional[str]) -> Dict[str, List[str]]:
    """
    Load a synonym mapping JSON file: { "token_or_phrase": ["alt1", "alt2", ...], ... }.
    """
    if not path:
        return {}
    with open(path, "r") as f:
        data = json.load(f)
    if not isinstance(data, dict):
        raise ValueError("synonyms_json must contain a JSON object mapping strings -> list[str].")
    out: Dict[str, List[str]] = {}
    for k, v in data.items():
        if not isinstance(k, str) or not isinstance(v, list) or not all(isinstance(x, str) for x in v):
            raise ValueError("synonyms_json must map string keys to list[str] values.")
        out[k] = v
    return out


def apply_synonyms(text: str, synonyms: Dict[str, List[str]], rng: random.Random) -> str:
    """
    Apply random synonym substitutions based on a mapping. Keys are matched case-insensitively as whole words
    (for single-token keys) or as raw substrings (for multi-word keys).
    """
    if not synonyms:
        return text

    out = text
    # Apply multi-word keys first (substring replacement).
    multi = sorted([k for k in synonyms.keys() if " " in k.strip()], key=len, reverse=True)
    for k in multi:
        alts = synonyms.get(k, [])
        if not alts:
            continue
        ...
        if pattern.search(out):
            out = pattern.sub(rng.choice(alts), out)

    # Apply single-word keys as whole-word replacements.
    single = [k for k in synonyms.keys() if " " not in k.strip()]
    for k in single:
        alts = synonyms.get(k, [])
        if not alts:
            continue
...
def default_synonyms() -> Dict[str, List[str]]:
    """
    Small built-in synonym set for quick experiments. Prefer passing --synonyms_json for attack-specific terms.
    """
    return {
        "red": ["crimson", "scarlet"],
        "yellow": ["golden", "amber"],
